...
function MediaPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Media library"
        subtitle="Reusable photos and videos for your posts."
        actions={
          q.data?.workspaceId ? (
            <MediaUploader
      ...
      {q.isLoading ? (
        ...
        <EmptyState title="No media yet" body="Upload photos or videos to reuse them across posts." />
      ) : (
          ...
          {(q.data?.items ?? []).map((m) => (
            <div key={m.id} className="card group relative overflow-hidden">
              <div className="aspect-square bg-[var(--color-mist-100)]">
                {m.kind === "video" ? (
                  m.url ? <video src={m.url} className="h-full w-full object-cover" muted /> : null
                ) : (
                  m.url ? <img src={m.url} alt={m.original_name ?? ""} className="h-full w-full object-cover" /> : null
                )}
              </div>
